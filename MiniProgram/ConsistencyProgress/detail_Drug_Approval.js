// ConsistencyProgress//detail_Drug_Approval.js
var app = getApp();
const util = require('../util/util.js');

const GetCond2Info_Drug_Approval = require('../config').ConsistencyProgress.GetCond2Info_Drug_Approval
const GetList_Drug_Approval = require('../config').ConsistencyProgress.GetList_Drug_Approval
Page({
  ...
  data: {
    ...
    Isbind: "",
    Modal_hidden: true,
    Width_Array: [{ Width: 25 }, { Width: 40 }, { Width: 35 }],
    Name_Array: [],
    Tool_Name_Array: [],
  ...
  Load_Cond2Info: function () {
    this.setData({
      cond_2nd: {
        ...
        HandleState_CFDAId: this.data.Sent_HandleState_CFDAId,
      }
    });

    var param1 = {};
    param1.url = GetCond2Info_Drug_Approval;
    var paramData1 = {};
    paramData1.type = -1;
    paramData1.pkey = "";
  ...
  Success_GetCond2Info: function (result, param) {
    ...
    result.Data.forEach(function (value, index, array) {
      ...
      if (value.type == 0) {
        ...
        for (var j = 0; j < this.data.Sent_HandleState_CFDAId.length; j++) {
          for (var k = 0; k < value.items.length; k++) {
            if (this.data.Sent_HandleState_CFDAId[j] == value.items[k].item_val) {
              value.items[k].open = true
              break
            }
          }
  ...
  Load_List: function () {
    ...
    //取得列表
    var param2 = {};
    param2.url = GetList_Drug_Approval;
    var paramData2 = {};
  ...
  Get_Field: function (e) {
    ...
    if (e.detail.Field_Type == 0) {
      ...
      this.setData({
        Sent_HandleState_CFDAId: Use_array,
      });
    }
    this.Load_Cond2Info();//取得二次筛选项
    this.Load_List();//加载列表
