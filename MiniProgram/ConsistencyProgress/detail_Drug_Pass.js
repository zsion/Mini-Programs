// ConsistencyProgress//detail_Drug_Pass.js
var app = getApp();
const util = require('../util/util.js');

const GetCond2Info_Drug_Pass = require('../config').ConsistencyProgress.GetCond2Info_Drug_Pass
const GetList_Drug_Pass = require('../config').ConsistencyProgress.GetList_Drug_Pass
...
Page({
  ...
  data: {
    ...
    Isbind: "",
    Modal_hidden: true,
    Width_Array: [{ Width: 30 }, { Width: 35 }, { Width: 35 }],
    Name_Array: [],
    Tool_Name_Array: [],
  ...
  Load_Cond2Info: function () {
    this.setData({
      cond_2nd: {
        ...
        CompanyName: this.data.Sent_CompanyName,
        CollectionCategoryId: this.data.Sent_CollectionCategoryId,
      }
    });

    var param1 = {};
    param1.url = GetCond2Info_Drug_Pass;
    var paramData1 = {};
    paramData1.type = -1;
    paramData1.pkey = "";
  ...
  Success_GetCond2Info: function (result, param) {
    ...
    result.Data.forEach(function (value, index, array) {
      ...
      if (value.type == 0) {
        ...
        for (var j = 0; j < this.data.Sent_CompanyName.length; j++) {
          for (var k = 0; k < value.items.length; k++) {
            if (this.data.Sent_CompanyName[j] == value.items[k].item_val) {
              value.items[k].open = true
              break
            }
          }
        ...
        for (var j = 0; j < this.data.Sent_CollectionCategoryId.length; j++) {
          for (var k = 0; k < value.items.length; k++) {
            if (this.data.Sent_CollectionCategoryId[j] == value.items[k].item_val) {
              value.items[k].open = true
              break
            }
          }
  ...
  Get_Field: function (e) {
    ...
    if (e.detail.Field_Type == 0) {
      ...
      this.setData({
        Sent_CompanyName: Use_array,
      });
    } else if (e.detail.Field_Type == 2) {
      this.setData({
        Sent_CollectionCategoryId: Use_array,
      });
    }
    this.Load_Cond2Info();//取得二次筛选项
    this.Load_List();//加载列表
